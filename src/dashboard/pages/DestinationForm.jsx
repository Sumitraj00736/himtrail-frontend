...
const inputCls =
  'w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand/30 focus:border-brand transition-all';
const textareaCls = `${inputCls} rounded-2xl resize-y`;

const slugFromHref = (href = '') =>
  href
    .replace(/^\/(trips|destinations)\//, '')
    .replace(/^\//, '')
    .toLowerCase()
    .trim();
...
const DestinationForm = () => {
  ...
  const [menuItemsByCountry, setMenuItemsByCountry] = useState({});
  ...
  const menuItemsForCountry = menuItemsByCountry[form.destination] || [];

  useEffect(() => {
    /* Dependencies: countries, categories, regions */
    api
      ...
      .then((res) => {
        ...
        if (Object.keys(data.regionsByCountry || {}).length) {
          setRegionsByCountry(data.regionsByCountry);
        }
      })
      .catch(() => {});

    /* Menu items for optional auto-fill (Destinations menu) */
    api
      .get('/dashboard/menus')
      .then((res) => {
        const menus = res.data.data || [];
        const destMenu =
          menus.find((m) => /destination/i.test(m.label)) ||
          menus.find((m) => m.style === 'mega');
        const itemsByCountry = {};
        (destMenu?.columns || []).forEach((col) => {
          const country = col.title?.trim();
          if (!country) return;
          itemsByCountry[country] = (col.items || []).filter((i) => i.label);
        });
        setMenuItemsByCountry(itemsByCountry);
      })
      .catch(() => {});
    ...
    api
      ...
      .then((res) => {
        ...
        setForm({
          ...
          menuHref: t.menuHref || `/destinations/${t.slug || ''}`,
        });
      })
      .catch(() => setMessage({ text: 'Failed to load destination.', type: 'error' }))
  ...
  /* Selecting a Destinations-menu item auto-fills title/slug and points public URL to /destinations/:slug */
  const handleMenuItemSelect = (href) => {
    if (!href) {
      setForm({ ...form, menuHref: '' });
      return;
    }
    const item = menuItemsForCountry.find((i) => i.href === href);
    const slug = slugFromHref(href) || slugFromHref(item?.label || '');
    const publicHref = `/destinations/${slug}`;
    setForm({
      ...form,
      menuHref: publicHref,
      title: item?.label || form.title,
      slug: slug || form.slug,
      destinationLabel: form.destinationLabel || form.destination,
    });
    /* Smooth scroll to basic info so the filled fields are visible */
    document.getElementById('destination-basic')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  };
  ...
  const submit = async (e) => {
    ...
    delete payload.menuHref;

    try {
      if (isNew) {
        await api.post('/dashboard/destinations', payload);
        setMessage({ text: '✅ Destination created successfully!', type: 'success' });
        setTimeout(() => navigate('/dashboard/destinations'), 1200);
      } else {
  ...
  const deleteDestination = async () => {
    if (!window.confirm('Delete this destination permanently? This cannot be undone.')) return;
    try {
      await api.delete(`/dashboard/destinations/${id}`);
      navigate('/dashboard/destinations');
    } catch {
      setMessage({ text: '❌ Failed to delete destination.', type: 'error' });
    }
  };
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex-grow">
          <h1 className="text-2xl font-bold text-slate-800 tracking-tight">
            {isNew ? '➕ Create New Destination' : '✏️ Edit Destination'}
          </h1>
          {!isNew && form.title && (
      ...
      <form onSubmit={submit} className="space-y-8">
        {/* ── Menu link (from Destinations menu) ── */}
        <section className="bg-brand/5 border border-brand/20 rounded-3xl p-6 space-y-4">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <h2 className="font-bold text-slate-700 text-base">Menu Dropdown</h2>
              <p className="text-slate-500 text-xs mt-1">
                Items from your <strong>Destinations</strong> menu appear here. Selecting one auto-fills
                title &amp; slug. Public menu clicks open <code className="text-[11px] bg-white px-1 rounded">/destinations/:slug</code>.
              </p>
            </div>
            <Link
              to="/dashboard/menus"
              className="text-xs font-bold text-brand hover:underline flex-shrink-0"
            >
              Manage Menus →
            </Link>
          </div>
          <div className="grid sm:grid-cols-2 gap-4">
            <Field label="Country (from menu columns)">
              <select
                className={inputCls}
            ...
            <Field label="Menu item (auto-fill form)">
              <select
                className={inputCls}
                value={
                  menuItemsForCountry.some((i) => {
                    const s = slugFromHref(i.href);
                    return `/destinations/${s}` === form.menuHref || i.href === form.menuHref;
                  })
                    ? menuItemsForCountry.find((i) => {
                        const s = slugFromHref(i.href);
                        return `/destinations/${s}` === form.menuHref || i.href === form.menuHref;
                      })?.href || ''
                    : ''
              ...
              >
                ...
                {menuItemsForCountry.map((item) => (
                  <option key={`${item.label}-${item.href}`} value={item.href}>
                    {item.label}
                  </option>
                ))}
              </select>
              {menuItemsForCountry.length === 0 && (
                <p className="text-[10px] text-amber-600 mt-1 font-medium">
                  No menu items for {form.destination}. Add them under Destinations → {form.destination} in Menus.
                </p>
              )}
            </Field>
          </div>
          {form.slug && (
            <div className="flex items-center gap-3 flex-wrap">
              <p className="text-xs text-slate-500 font-medium">
                Public page:{' '}
                <span className="text-brand font-bold">/destinations/{form.slug}</span>
              </p>
              <Link
                to={`/destinations/${form.slug}`}
                target="_blank"
                rel="noreferrer"
                className="text-xs font-bold text-brand px-3 py-1.5 rounded-lg bg-white border border-brand/20 hover:bg-brand/5 transition-colors"
              >
                Open page →
              </Link>
              <p className="text-[10px] text-slate-400">
                Tip: set the menu item href to <strong>/destinations/{form.slug}</strong> so navbar clicks open this page.
              </p>
            </div>
          )}
        </section>
        ...
        {/* ── Basic Info ── */}
        <section id="destination-basic" className="bg-slate-50 border border-slate-200 rounded-3xl p-6 space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <Field label="Category">
              ...
              <p className="text-[10px] text-slate-400 mt-0.5 font-medium">From Dependencies</p>
            </Field>
            <Field
              label={
            ...
            >
              ...
              <p className="text-[10px] text-slate-400 mt-0.5 font-medium">
                📍 {availableRegions.length} regions for {form.destination}
              </p>
            </Field>
          </div>
        </section>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Field label="Trip Grade">
              <input
                ...
                value={form.tripGrade}
                onChange={(e) => setForm({ ...form, tripGrade: e.target.value })}
              />
            </Field>
          </div>
        ...
        <section className="bg-slate-50 border border-slate-200 rounded-3xl p-6">
          <h2 className="font-bold text-slate-700 text-base mb-1">Homepage Carousels</h2>
          <p className="text-slate-400 text-xs mb-4">
            Choose where this destination appears on the public homepage.
          </p>
                ...
                <div>
                  ...
                  <div
                    className={`text-[10px] font-normal mt-0.5 ${
                      form.destinationSections.includes(value) ? 'text-white/70' : 'text-slate-400'
                    }`}
                  >
                    {desc}
                  </div>
            ...
            {form.itinerary.map((it, idx) => (
              <div key={idx} className="grid grid-cols-[64px_1fr_2fr_auto] gap-2 items-start">
                <input
                  ...
                  placeholder="Day"
                  onChange={(e) => {
                    const itinerary = form.itinerary.map((x, i) =>
                      i === idx ? { ...x, day: Number(e.target.value) } : x
                    );
                ...
                <button
                  type="button"
                  className="text-red-400 hover:text-red-600 text-xs font-bold px-2 py-2.5"
                  onClick={() => {
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          ...
          <section className="bg-slate-50 border border-slate-200 rounded-3xl p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                type="button"
                onClick={() => setForm({ ...form, excluded: [...form.excluded, ''] })}
                className="text-xs font-bold text-brand hover:underline"
              >
                + Add
              </button>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              ...
              onClick={() =>
                setForm({
                  ...form,
                  dates: [...form.dates, { startDate: '', endDate: '', status: '', price: '' }],
                })
              }
              className="text-xs font-bold text-brand hover:underline"
            >
            ...
            {form.dates.map((row, idx) => (
              <div key={idx} className="grid grid-cols-[1fr_1fr_1fr_1fr_auto] gap-2 items-center">
                <input
                  ...
                  onChange={(e) => {
                    const dates = form.dates.map((x, i) =>
