...
const BestSellersAdmin = () => {
  ...
  const load = async () => {
    const res = await api.get('/dashboard/best-sellers');
  ...
  const create = async (e) => {
    e.preventDefault();
    await api.post('/dashboard/best-sellers', form);
    setForm({ tag: '', title: '', duration: '', price: '', oldPrice: '', reviews: '', image: '', order: 0 });
    load();
  };

  const remove = async (id) => {
    await api.delete(`/dashboard/best-sellers/${id}`);
    load();
  };

  return (
    <div>
      <h1 className="text-xl font-semibold text-[#243b75]">Best Sellers</h1>
