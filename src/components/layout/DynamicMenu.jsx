...
const DynamicMenu = () => {
  ...
  return (
    <>
      ...
      {/* Mobile Menu */}
      {mobileOpen && (
        <div className="xl:hidden bg-white shadow-md rounded-lg p-4 mt-2 space-y-4">
          {menus.map((menu) => (
            <div key={menu._id}>
              <p className="font-semibold text-forest-800">{menu.label}</p>
              {menu.columns.map((col) => (
                <div key={col.title} className="pl-4 mt-2">
                  <p className="text-sm font-medium text-forest-700 mb-1">{col.title}</p>
                  <ul className="space-y-1 text-sm text-forest-700">
                    {col.items.map((item) => (
                      <li key={item.label}>
                        <Link
                          to={item.href}
                          className="hover:text-forest-800 block"
